...
func create_white_ball() -> void:
	var prefab: PackedScene = load("res://whiteball.tscn")
	var instance: RigidBody2D = prefab.instantiate() as RigidBody2D
	get_window().add_child.call_deferred(instance)
	instance.position = Vector2(table.scale.x / 2, table.scale.y / 2)
	white_ball = instance
	print("Spawned")
